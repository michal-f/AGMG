...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ cpr_upload_dir+"/"+file.name + '&task=cpr_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	cpr_refresh_images_table(cpr_current_album_id);
...
function cpr_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=cpr_get_albums_table', function(res)
	{
		cpr_set_data_tables();
		if (res.status == 'ok') 
		{
...
function cpr_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=cpr_get_albums_images_table&album_id='+album_id, function(res)
	{
		cpr_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'cpr_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'cpr_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'cpr_reorder_image' )
		{
			cpr_refresh_images_table(album_id);
		}
		if( task == 'cpr_reorder_album' )
			cpr_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function cpr_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', cpr_submit_bulk);
...
function cpr_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'cpr_bulk_delete_albums' || task == 'cpr_bulk_enable_albums' || task == 'cpr_bulk_disable_albums')
	{
	...
	else if( task == 'cpr_bulk_delete_images' || task == 'cpr_bulk_enable_images' || task == 'cpr_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = cpr_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: cpr_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: cpr_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: cpr_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: cpr_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : cpr_url+"/js/swfupload/js/swfupload.swf",
				upload_url: cpr_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":cpr_upload_dir, "secw":cpr_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: cpr_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.cpr_delete_album').live('click', cpr_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', cpr_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', cpr_disable);
