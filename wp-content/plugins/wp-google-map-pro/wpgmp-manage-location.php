...
add_action('admin_init','wpgmp_export_csv');

function wpgmp_export_csv()
{
	global $wpdb;	

  if( !empty($_POST['action']) )
  {  	
    if( $_POST['action'] == 'export_csv' || $_POST['action'] == 'export_xml' || $_POST['action'] == 'export_excel' || $_POST['action'] == 'export_json' )
  	{
  		$location_data = array();
  		$ids = $_POST['location'];
  		...
  		{
  		    foreach($ids as $id) {
  			    $query = $wpdb->prepare("SELECT * FROM ".$location_table." WHERE location_id=%d",$id);
  			    $location_data[] = $wpdb->get_row($query);
  		    }
  		}
  		else
  		{
  			  $query = $wpdb->prepare("SELECT * FROM ".$location_table." ",null);
  			  $all_locations = $wpdb->get_results($query);
  		...
  		wpgmp_location_export($_POST['action'],$location_data, "locations-export", true);
  		exit;
  	}
  }  
}
...
class Wpgmp_Location_Table extends WP_List_Table {
    ...
    global $status, $page,$wpdb;
		...
		if($_GET['page']=='wpgmp_manage_location' && !empty($_POST['s']) )
		{
		  $query = "SELECT * FROM ".$wpdb->prefix."map_locations WHERE location_title LIKE '%".$_POST['s']."%' OR location_address LIKE '%".$_POST['s']."%' OR location_latitude LIKE '%".$_POST['s']."%' OR location_longitude LIKE '%".$_POST['s']."%'";
		}
		else
		{
  ...
  function no_items() {
    _e( 'No Records for Manage Locations.' ,'wpgmp_google_map');
  }
...
function usort_reorder( $a, $b ) {
  ...
  $orderby = ( ! empty( $_GET['orderby'] ) ) ? $_GET['orderby'] : 'location_title';
...
if( !empty($_GET['action']) && $_GET['action']=='delete' && !empty($_GET['location']) )
{
...
if( !empty($_POST['action']) && $_POST['action']=='delete' && !empty($_POST['location']) )
{
...
array( 'location_id' => $_GET['location'] ) 
	...
	$success = __( 'Location updated successfully. You can manage your locations <a href="'.admin_url('admin.php?page=wpgmp_manage_location').'">here</a>.', 'wpgmp_google_map' );
	}
}


if( !empty($_GET['action']) && $_GET['action']=='edit' && !empty($_GET['location']) )
{
...
<form method="post">
  ...
  <p class="description"> <?php _e('Enter here the longitude.', 'wpgmp_google_map')?></p></div>
   ...
   <div class="col-md-7">    <p class="description"><input type="checkbox" name="googlemap_draggable" value="true"<?php checked($user_record->location_draggable,'true') ?>/>
    ...
    <div class="row">
   <div class="col-md-2">    <label for="title"><?php _e('Disable Infowindow', 'wpgmp_google_map')?></label></div>
   <div class="col-md-7">     <p class="description"><input type="checkbox" name="googlemap_clickable" value="false"<?php checked($infowindow_settings['hide_infowindow'],'false') ?>/>
   <?php _e('Do you want to disable infowindow for this location?', 'wpgmp_google_map')?></p></div>
    </div>
   ...
   <div class="col-md-7"> 
    ...
    <input type="submit" name="update_location" id="submit" class="btn btn-lg btn-primary" value="<?php _e('Update Locations', 'wpgmp_google_map')?>">
	</div>
    </div> 
</form>
</div>
