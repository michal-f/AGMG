...
function dslc_filter_content( $content ) {
	...
	if ( ( $currID == $realID && in_the_loop() && $dslc_should_filter ) || is_archive() ) {		
...
function dslc_modules_section_front( $atts, $content = null ) {
	...
	$output = '
		<div id="'. $section_id .'" class="dslc-modules-section ' . $a_container_class . $parallax_class . $section_class .'" style="' . $section_style . '">
	...
	echo '<style type="text/css">';
		...
		if ( $composer_code ) {
			...
			foreach ( $dslc_googlefonts_array as $googlefont) {
				...
				if ( $googlefont != '' ) {
					$googlefonts_output .= '@import url("//fonts.googleapis.com/css?family=' . $googlefont . ':100,200,300,400,500,600,700,800,900&subset=latin,latin-ext,cyrillic,cyrillic-ext"); ';
				}
			}	
