...
function wpgmp_display_map($atts){
 ...
 if(is_array($marker_array)) {
	 foreach($marker_array as $marker){
		...
		$map->addMarker('','',$explode_marker[0],$explode_marker[1],$clickable,'',$explode_marker[2],$icon,'',$draggable,'',$group_id='');
	 }
 }
 ...
 if(is_array($address_array)) {
	  foreach($address_array as $address) {
		...
		$map->addMarker('','',$lat,$lng,$clickable,'',$explode_address[1],$icon,'',$draggable,'',$group_id='');
	 }
 }
