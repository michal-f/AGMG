...
function srp_widgets_init() {
	...
	// Registering widget.
	register_widget( 'WDG_SpecialRecentPostsPro' );
}
...
function srp_check_plugin_compatibility() {
	...
	$errorMessages = array(
		'phpversion'      => sprintf( __( "%1\$sSpecial Recent Posts Error!%2\$s%3\$s You're running an old version of PHP. In order for this plugin to work, you must enable your server with PHP support version 5.0.0+. Please contact your hosting/housing company support, and check how to enable it.", SRP_TRANSLATION_ID ), '<strong>', '</strong>', '<br />' ),
		'gd_info'         => sprintf( __( "%1\$sSpecial Recent Posts Error!%2\$s%3\$s GD libraries are not supported by your server. Please contact your hosting/housing company support, and check how to enable it. Without these libraries, thumbnails can't be properly resized and displayed.", SRP_TRANSLATION_ID ), '<strong>', '</strong>', '<br />' ),
		'post-thumbnails' => sprintf( __( "%1\$sSpecial Recent Posts Warning!%2\$s%3\$s Your theme doesn't support post thumbnail. The plugin will keep on working with first post images only. To enable post thumbnail support, please check the Wordpress documentation", SRP_TRANSLATION_ID ), '<strong>', '</strong>', '<br />' ),
		'cache-exists'    => sprintf( __( "%1\$sSpecial Recent Posts Warning!%2\$s%3\$s The Cache folder does not exist!. In order to use caching functionality you have to manually create a folder named 'cache' under the special-recent-posts/ directory.", SRP_TRANSLATION_ID ), '<strong>', '</strong>', '<br />' ),
		'cache-writable'  => sprintf( __( "%1\$sSpecial Recent Posts Warning!%2\$s%3\$s The Cache folder is not writable. In order to use caching functionality you have to set the correct writing permissions on special-recent-posts/cache/ folder. E.G: 0755 or 0775", SRP_TRANSLATION_ID ), '<strong>', '</strong>', '<br />' )
	);

    // Checking current PHP version.
    if ( version_compare( phpversion(), SRP_REQUIRED_PHPVER ) == '-1' ) {
...
function srp_admin_menu() {
	...
	add_menu_page( 'Special Recent Posts PRO - ' . __( 'General Settings', SRP_TRANSLATION_ID ), 'SRP PRO', 'install_plugins', 'srp-pro-settings', 'srp_admin_menu_options', SRP_PLUGIN_URL . SRP_IMAGES_FOLDER . 'wp-menu-icon.png' );

	//create nu items
	add_submenu_page( 'srp-pro-settings', 'Special Recent Posts PRO - ' . __( 'General Settings', SRP_TRANSLATION_ID ), __( 'General Settings', SRP_TRANSLATION_ID ), 'install_plugins', 'srp-pro-general-settings', 'srp_admin_menu_options' );
...
function srp_admin_enqueue_scripts( $hook ) {
	...
	switch ( $hook ) {
		...
		case 'widgets.php':
			...
			// Enqueuing WP color picker CSS.
			wp_enqueue_style( 'wp-color-picker' );        

			// Enqueuing WP color picker JS library.
			wp_enqueue_script( 'wp-color-picker' );

			// Enqueuing the underscore.js framework.
			wp_enqueue_script( 'underscore' );
			
		break;

		// The SRP admin settings page.
		case 'toplevel_page_srp-pro-settings':
		case 'srp-pro_page_srp-pro-general-settings':
...
<div class="wrap">
	...
	<div id="srp-admin-container">
		...
		<div class="srp-admin-panel-header">
			...
			<!-- END SRP admin header logo -->

			<!-- BEGIN SRP admin header title -->
			<h2>
				<?php _e( 'Special Recent Posts PRO - General Settings', SRP_TRANSLATION_ID ); ?>
			</h2>
			<!-- END SRP admin header title -->

		</div>
		<!-- END SRP admin page header -->
		
		<!-- BEGIN SRP admin page header description -->
		<div class="srp_settings_description">
			<?php printf( __( '%1$sWelcome to the SRP PRO General Settings Page.%2$s%3$sIn this area you can configure all the main settings for the Special Recent Posts PRO plugin. Please keep in mind that these are basic options; further special customization options apply for each widget instance, shortcode or PHP code to ensure an high level of customization. Go to your Wordpress widgets page and drag the Special Recent Posts PRO widget onto one of your draggable areas to see all the additional options available.', SRP_TRANSLATION_ID ), '<strong>', '</strong>', '<br />' ); ?>
		</div>
			...
			<div class="srp-tab">
				...
				<div id="srp-tab-2" class="metabox-holder srp-tab-content">
				    ...
				    <div class="wrap">
				        ...
				        <!-- BEGIN Custom CSS Editor Description -->
				        <div class="srp-label-description">
				        	<?php
				        		
				        	?>
				        </div>
				        <!-- END Custom CSS Editor Description -->
				...
				<div id="srp-tab-4" class="metabox-holder srp-tab-content srp-tab-plugin-info">
					...
					<h3>
						<?php _e( 'Special Recent Posts PRO Edition', SRP_TRANSLATION_ID ); ?>
					</h3>
 ...
 * get_image_sizes()
 *
 * This function retrieves all the current active images sizes set in the WP theme function file.
 *
 * @author Luca Grandicelli <lgrandicelli@gmail.com>
 * @copyright (C) 2011-2014 Luca Grandicelli
 * @package special-recent-posts-pro
 * @version 3.0.3
 * @global $_wp_additional_image_sizes The globa additional WP images sizes.
 * @param array $size The available thumbnail sizes
 * @return boolean/array It could return the array containing all the WP image sizes or false if none is set.
 */
function get_image_sizes( $size = '' ) {

	// Importing global WP image sizes.
    global $_wp_additional_image_sizes;

    // Setting up the new sizes array.
    $sizes = array();

    // Fetch sizes.
    $get_intermediate_image_sizes = get_intermediate_image_sizes();

    // Creating the full array with sizes and crop info
    foreach( $get_intermediate_image_sizes as $_size ) {

    	// Checking if the current size is part of the default wp ones.
    	if ( in_array( $_size, array( 'thumbnail', 'medium', 'large' ) ) ) {
    		...
    		// Loading the new custom WP image size.
    		$sizes[ $_size ] = array( 
    ...
    // Checking if this function should retrieve one size.
    if ( $size ) {

    	// Checking if the selected size exists.
        if( isset( $sizes[ $size ] ) ) {
