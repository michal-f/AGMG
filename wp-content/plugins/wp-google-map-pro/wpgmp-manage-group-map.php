...
class Wpgmp_Manage_Group_Table extends WP_List_Table {
    ...
    global $status, $page,$wpdb;
		...
		if( $_GET['page']=='wpgmp_google_wpgmp_manage_group_map' && !empty($_POST['s']) )
		{
  ...
  function no_items() {
    _e( 'No Records for Marker Category.' ,'wpgmp_google_map');
  }
...
function get_columns(){
        $columns = array(
			...
			'group_map_title'      => __( 'Category Title', 'wpgmp_google_map' ),
			'group_marker'      => __( 'Category Marker', 'wpgmp_google_map' ),
			'group_added'      => __( 'Category Added', 'wpgmp_google_map' ),
        );
         return $columns;
    }
...
global $wpdb; 
if( !empty($_GET['action']) && $_GET['action']=='delete' && !empty($_GET['group_map']) )
{
...
if( !empty($_POST['action']) && $_POST['action'] == 'delete' && !empty($_POST['group_map']) )
{
...
if( isset($_POST['update_group_map']) && $_POST['update_group_map']=='Update Marker Category' )
{
...
if( empty($_POST['upload_image_url']) )
{
...
$success= __( 'Category Updated Successfully.', 'wpgmp_google_map' );
	}
}
?>
<?php
if( !empty($_GET['action']) && $_GET['action']=='edit' && !empty($_GET['group_map']) )
{
...
<h3><span class="glyphicon glyphicon-asterisk"></span><?php _e('Edit Marker Category', 'wpgmp_google_map')?></h3>
...
<fieldset>
   ...
   <div class="row">
    <div class="col-md-3">   <label for="title"><?php _e('Category Title', 'wpgmp_google_map')?>&nbsp;<span style="color:#F00;">*</span></label>
    </div>
   <div class="col-md-7">
    ...
    <p class="description"><?php _e('Enter here the category title.', 'wpgmp_google_map')?></p></div>
    </div>
...
<div class="row">
 <div class="col-md-3">  </div><div class="col-md-7"> 
<input type="submit" name="update_group_map" id="submit" class="btn btn-lg btn-primary" value="<?php _e('Update Marker Category', 'wpgmp_google_map')?>"></div>
...
<div class="wpgmp-wrap">  
<div class="col-md-12">
<div id="icon-options-general" class="icon32"><br></div><h3><span class="glyphicon glyphicon-asterisk"></span><?php _e('Manage Marker Categories', 'wpgmp_google_map')?></h3>
<?php
