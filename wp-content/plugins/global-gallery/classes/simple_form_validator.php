...
class simple_fv {
	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_val') {
			if((float)$index_val < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('minimum value is', 'gg_ml').' '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_val') {
			if((float)$index_val > (int)$val) {
				if(!$test) {
					#$this->errors['value'][] = $label;
					$this->custom_error[$label] = __('maximum value is', 'gg_ml').' '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('minimum', 'gg_ml').' '.$val.' '.__('characters allowed', 'gg_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('maximum', 'gg_ml').' '.$val.' '.__('characters allowed', 'gg_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'right_len') {
			if(strlen($index_val) != (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('has to be long', 'gg_ml').' '.$val.' '.__('characters', 'gg_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_array') {
			if(!is_array($index_val) || count($index_val) < $val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('Select').' '.$val.' '.__('options', 'gg_ml');
				}
				return false;	
			}
		}
		...
		if($type == 'max_array' && is_array($index_val)) {
			if(count($index_val) > $val) {
				if(!$test) {
					#$this->errors['array_count'][] = $label;
					$this->custom_error[$label] = __('Maximum', 'gg_ml').' '.$val.' '.__('options allowed', 'gg_ml');
				}
				return false;	
			}
		}
	...
	private function errorTranslate($type, $label_array) {
		...
		switch($type) {
			case 'required' : 
				(count($label_array) > 1) ? $message .= __('are required', 'gg_ml') : $message .= __('is required', 'gg_ml');
				break;
			
			case 'wrong_int' : 
				(count($label_array) > 1) ? $message .= __('are not valid integers', 'gg_ml') : $message .= __('is not a valid integer', 'gg_ml');
				break;
				
			case 'wrong_float' : 
				(count($label_array) > 1) ? $message .= __('are not valid floating numbers', 'gg_ml') : $message .= __('is not a valid floating number', 'gg_ml');
				break;	
				
			case 'wrong_mail' : 
				(count($label_array) > 1) ? $message .= __('are not valid e-mail addresses', 'gg_ml') : $message .= __('is not a valid e-mail address', 'gg_ml');
				break;		
				
			case 'wrong_date' : 
				(count($label_array) > 1) ? $message .= __('are not valid dates', 'gg_ml') : $message .= __('is not a valid date', 'gg_ml');
				break;		
				
			case 'wrong_time' : 
				(count($label_array) > 1) ? $message .= __('are not valid times', 'gg_ml') : $message .= __('is not a valid time', 'gg_ml');
				break;
				
			case 'wrong_url' : 
				(count($label_array) > 1) ? $message .= __('are not valid urls', 'gg_ml') : $message .= __('is not a valid url', 'gg_ml');
				break;	
				
			case 'wrong_hex' : 
				(count($label_array) > 1) ? $message .= __('are not valid hexadecimal colors', 'gg_ml') : $message .= __('is not a valid hexadecimal color', 'gg_ml');
				break;	
			
			case 'wrong_ip' : 
				(count($label_array) > 1) ? $message .= __('are not valid IP addresses', 'gg_ml') : $message .= __('is not a valid IP address', 'gg_ml');
				break;	
				
			case 'wrong_zip' : 
				(count($label_array) > 1) ? $message .= __('are not valid ZIP codes', 'gg_ml') : $message .= __('is not a valid ZIP code', 'gg_ml');
				break;
				
			case 'wrong_tel' : 
				(count($label_array) > 1) ? $message .= __('are not valid telephone numbers', 'gg_ml') : $message .= __('is not a valid telephone number', 'gg_ml');
				break;		
					
			case 'wrong_type' : $message .= __('invalid data inserted', 'gg_ml');
				break;	
				
			case 'allowed' : 
				(count($label_array) > 1) ? $message .= __('values are not between the allowed', 'gg_ml') : $message .= __('value is not between the allowed', 'gg_ml');
				break;
				
			case 'forbidden' : 
				(count($label_array) > 1) ? $message .= __('values are between the forbidden', 'gg_ml') : $message .= __('values is between the forbidden', 'gg_ml');
				break;
				
			case 'not_equal' : $message .= __("the value doesn't match", 'gg_ml'); 
				break;
				
			case 'size' : $message .= __('file size is wrong', 'gg_ml');
				break;						
		}	
