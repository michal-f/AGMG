...
jQuery( document ).ready( function( $ ) {
	 ...
	 */
	var pt_search_replace = function ( $el, searchFor ) {
		...
		$el.html(
			$el.html().replace(
				expression,
				'<img src="' + BuildPressAdminVars.pathToTheme + '/assets/images/widget-title-logo.png" class="proteusthemes-widget-logo" style="position: relative; top: 2px; margin-right: 4px;" /> '
			)
		);
	}

	// for appearance > widgets
	$( '.panel-type-wrapper > h3, .widget-title > h4' ).each( function() {
		pt_search_replace( $( this ) );
	} );

	// same, but inside page builder
	$( 'body' ).on( 'refreshcells panelsdone', function () {
		$( this ).find( '.title > h4, .ui-dialog-title' ).each( function () {
			pt_search_replace( $( this ) );
		} )
	} );

	// same, but inside customizer: [PT] Theme Options
	$( '#accordion-panel-panel_proteusthemes > .accordion-section-title, #accordion-panel-panel_proteusthemes .panel-title' ).each( function () {
		pt_search_replace( $( this ), '\\[PT\\] ' );
	} );
...
ptMapsLocations.locationsView = Backbone.View.extend( {
	...
	appendOne: function ( location ) {
		var renderedLocation = new ptMapsLocations.locationView( {
			model:    location,
			template: _.template( jQuery( '#js-pt-location-' + this.widgetId ).html() ),
		} ).render();
...
var repopulateLocations = function ( locationsJSON, widgetId ) {
	...
	locationsView.locations.add( locationsJSON );
};
...
ptTestimonials.testimonialsView = Backbone.View.extend( {
	...
	appendOne: function ( testimonial ) {
		var renderedTestimonial = new ptTestimonials.testimonialView( {
			model:    testimonial,
			template: _.template( jQuery( '#js-pt-testimonial-' + this.widgetId ).html() ),
		} ).render();
...
var repopulateTestimonials = function ( testimonialsJSON, widgetId ) {
	...
	testimonialsView.testimonials.add( testimonialsJSON );
};